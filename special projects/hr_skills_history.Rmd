...
```{r hr_diff_m, echo=FALSE, eval=FALSE}
hr_diff_m <- hr %>%
    group_by(Match) %>%
...
The mean difference between pre and post survey is `r pander(mean(hr$hr.diff))`The table below shows that those in the program for multiple years did not perform better. 
...
```{r shapiro.test, echo=FALSE, eval=FALSE}
