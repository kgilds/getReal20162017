...
```{r}
read_success_m <- read_history %>%
    filter(final >= 2, Grade_Change >=0) %>%
    group_by(council) %>%
    count(council) %>%
    mutate(Percent = n / sum (n))
  
read_success_m$Percent <- percent(read_success_m$Percent) 
    
kable(read_success_m)
```
...
```{r read_increase_tbl, echo=FALSE, eval=TRUE}
