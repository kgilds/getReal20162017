...
The number of students in the dataset is `r nrow(susp_his)` and the number of matched students acorss the two years is `r nrow(susp_dupes)`. 
...
Over the course of two years `r (tot_suc)` students  have had a successful outcome and `r (no_per) ` never received a suspension. A successful outcome is never receiving a suspension and decreasing amount of suspensions. However, there is a segment of students  around 8 percent that on average increased there amount of suspesnions across the school year. 
...
```{r susp_avg_m, echo=FALSE, eval=FALSE}
susp_avg_m <- susp_his %>%
    filter(baseline != 0, final !=0) %>%
    group_by(Match)%>%
    summarise(avg_diff =mean(susp_diff) )%>%
