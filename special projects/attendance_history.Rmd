...
attendance_1516 <- attendance_1516 %>%
     ...
     select(1,3, 4, 6,7) #select variables of interest 
...
attendance_1415 <- attendance_1415%>% 
    ...
    select(1,4,3, 5,6)#select column of interest
...
attendance_avg <- attendance_his %>%
    summarise(Att_diff = mean(Att_diff))
...
abs_plot <- ggplot(att_sum_c,(aes(x = Att_diff, y =Council)))
abs_plot + geom_count()
