...
###########################Read Data#############################################
rawdata <- read_csv("girl_survey_10092016.csv", skip=1)
dim(rawdata)
...
### Data Entry Status for Pre Survey

```{r data_source_tbl}

pre_girl_status <- preUnique %>% 
    select(council) %>%
    mutate("Data Source" = "Pre-Student")


pre_girl_status

pre_girl_tbl <- with(pre_girl_status,table(council))

pre_girl_tbl <- addmargins(pre_girl_tbl)

pre_girl_tbl
```


```{r student_file_function}


student_file <- function(x) {
    student_file <- filter(preUnique, council == x)
    write.csv (student_file, file="student_survey.csv", row.names=FALSE)
...
student_file("Southeast Council")
...
student_file("Panhandle Council")
...
student_file("West Central Council")
