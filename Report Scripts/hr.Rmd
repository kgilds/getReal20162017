...
hr <-readRDS("hr_2017.rds") #read hr rds file
...
```{r matched_hr, echo=FALSE}
matched_hr <- hr  # set up for hr success function in narraive
...
The number of pre and post survey that could be matched is **`r nrow(hr)`**, and the percent of students with a successful outcome is: **`r p_acheived`**
...
hr_bar_pre <-qplot(hr$Pre >=4.45, geom="bar", ylim=c(0, 300), main="Pre survey Results", xlab="Achievment at High Level")

hr_bar_post <-qplot(hr$Post >=4.45, geom ="bar",ylim=c(0,300), main="Post Survey Results", xlab="Achievment at High Level")
...
healthy_change_p <- ggplot(hr,(aes(x=hr.diff, y=Success)))
healthy_change_p + geom_count() + geom_vline(xintercept = 0, col="red") + labs(x="Pre and Post Difference", y="High Level", title="Healthy Relationship Survey Results")
...
```{r}
pander(wilcox.test(hr$Pre, hr$Post, "less"))
```



```{r}
cohen.d(hr$Post, hr$Pre, paired=TRUE)
```
